...
> implements IBaseRepository<TEntity, TDocument> {
  ...
  async findByIdAndUpdate(id: string, data: Partial<TEntity>): Promise<TEntity | null> {
    const doc = await this.model.findByIdAndUpdate(id, { $set: data as any }, { returnDocument: "after" })
    if (!doc) return null
    return this.toDomain(doc)
  }
  
  async findById(id: string, options?: QueryOptions): Promise<TEntity | null> {
  let doc = await this.model.findById(id).select(options?.select ? options?.select : "")
  ...
  async findOneBy(filter: string | Record<string, any>): Promise<TEntity | null> {
  ...
  async updateOneById(
    ...
    const query = typeof filter === "string" ? { _id: filter } : filter;
    const doc = await this.model.findOneAndUpdate(query as Record<string, any>, update, { new: false });
    if (!doc) return null;
    return this.toDomain(doc);
  }
  
  async updateMany(filter: string | Record<string, any>, update: UpdateQuery<TDocument>): Promise<void> {
    const query = typeof filter === "string" ? { _id: filter } : filter;
