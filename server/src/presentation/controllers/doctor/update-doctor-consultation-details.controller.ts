...
import type { IDoctorConsultationUseCase } from "../../../application/repositories/doctor/i-doctor-consultation.usecase.ts";
...
export class UpdateDoctorConsultationDetailsController {
  ...
  async handle(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const response = await this._doctorConsultation.execute({
        ...data,
        userId,
      });
