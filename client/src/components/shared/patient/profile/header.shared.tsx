...
import AddProfileModal from "./add-profile-modal.shared";
          ...
          <div className="absolute right-0 mt-2 w-52 rounded-xl border border-white/10 bg-[#1a1a2e] shadow-xl overflow-hidden z-50">
            <div className="p-1">
              ...
              {patients.map((patient) => {
                const isActive =
                  patient.patient?.id === activePatient!.patient?.id;
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="text-left">
                        <p
                          className={`text-sm leading-tight ${isActive ? "text-white font-medium" : "text-white/60 group-hover:text-white/80"}`}
                        >
                    ...
                    {isActive && (
                      <Check size={13} className="text-white/70 shrink-0" />
                    )}
                  </button>
                );
              })}
            ...
            <div className="border-t border-white/8 p-1">
              <button
                className="w-full flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-white/8 transition-colors group"
